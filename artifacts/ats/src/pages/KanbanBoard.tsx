...
import { useGetKanban, useUpdateCandidateStatus, CandidateStatus, getGetKanbanQueryKey } from "@workspace/api-client-react";
  ...
  const handleDrop = (e: React.DragEvent, status: CandidateStatus) => {
    ...
    updateStatus.mutate(
      { id, data: { status } },
      {
        ...
        onError: () => {
          toast({ variant: "destructive", title: "Update failed", description: "Could not move candidate." });
    ...
    return (
      <div className="space-y-6 h-full flex flex-col">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Drag and drop candidates across stages</p>
        </div>
        <div className="flex gap-4 overflow-x-auto pb-4 flex-1">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="min-w-[300px] w-[300px] flex-shrink-0 bg-muted/30 rounded-xl p-3 h-full">
          ...
          >
            <div className="p-3 border-b border-border/50 flex items-center justify-between font-medium">
              <div className="flex items-center gap-2">
                <span className="capitalize text-sm font-semibold">{col.status}</span>
              ...
              {col.candidates.map((candidate) => (
                <div
                  ...
                  className={`bg-card border border-border shadow-sm rounded-lg p-4 cursor-grab hover:border-primary/50 transition-colors ${
                    draggedId === candidate.id ? "opacity-50 border-primary" : ""
                  }`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <Link href={`/candidates/${candidate.id}`} className="font-medium text-sm hover:text-primary transition-colors">
                      {candidate.name}
                    </Link>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <div className="flex gap-1 overflow-hidden">
                      {candidate.skills?.slice(0, 2).map((skill, i) => (
                        <span key={i} className="px-1.5 py-0.5 bg-muted rounded truncate max-w-[80px]">
                          {skill}
                        </span>
                      ))}
                    </div>
